...
find "$folder"/processing/"$dataset"/ -name "*.json" | while read line; do
  ...
  <"$line" jq '.[2]|[.]|map([.x, .y]| transpose|.[]|{x:.[0],y:.[1]})'  | mlr --j2c label date,valore >"$folder"/processing/"$dataset"/operatori_"$nome".csv
done

# sintomatici altri
find "$folder"/processing/"$dataset"/ -name "*.json" | while read line; do
  nome=$(basename "$line" ".json")
  <"$line" jq '.[3]|[.]|map([.x, .y]| transpose|.[]|{x:.[0],y:.[1]})'  | mlr --j2c label date,valore >"$folder"/processing/"$dataset"/altri_"$nome".csv
done
...
mlr --csv reshape -s tipo,valore then unsparsify then sort -f regione,date "$folder"/processing/iss_opsan.csv >"$folder"/processing/iss_opsan_wide.csv
...
# creazione versione wide
mlr --csv reshape -s tipo,valore then unsparsify then sort -f regione,date "$folder"/processing/iss_opsan_altri.csv >"$folder"/processing/iss_opsan_altri_wide.csv
