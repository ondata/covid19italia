...
for i in $(seq 0 "$numeroClassi"); do
  ...
  numerorighe=$(cat "$folder"/tmp_classe_"$i".csv | tail -n +2 | wc -l)
  mlr --ocsv seqgen --step 6 --start 0 --stop "$numerorighe" -f id then put '$code=$id' >>"$folder"/rawdata/tmp_classe_"$i"_join.csv
  mlr --csv join --ul -j id -f "$folder"/tmp_classe_"$i".csv then unsparsify then sort -n id then fill-down -f code then cat -n -g code "$folder"/tmp_classe_"$i"_join.csv >"$folder"/rawdata/tmp_classe_"$i"_out.csv
done

mlr --csv cat "$folder"/rawdata/tmp_classe_*_out.csv >"$folder"/processing/incidenzaInizio.csv

#  jq <rawdata/htmlwidget-0971c37dd630e105e8c2.json '.x.data['"$i"']' | mlr --j2c reshape -r "^[xy]:" -o item,value then cut -f item,value then nest --explode --values --across-fields --nested-fs ":" -f item then reshape -s item_1,value then put '$classe='"$i"'' >tmp_classe_"$i".csv
#jq <rawdata/htmlwidget-0971c37dd630e105e8c2.json '.x.data['"$i"']' | mlr --j2c reshape -r "^[xy]:" -o item,value then cut -f item,value then nest --explode --values --across-fields --nested-fs ":" -f item then reshape -s item_1,value then put '$classe='"$i"'' >tmp_classe_"$i".csv

# mlr --csv join --ul -j id -f esempioa.csv then unsparsify then sort -n id then fill-down -f code then cat -n -g code join.csv

#python3 "$folder"/script_v2.py
